...
---------------------------------------
Purpose :
		- Creating stored procedure to load data after cleaning 
		  from Bronze Layer to silver Layer
		- We will make a full load 
		- The approch use is : 
						- Truncate tables data
						- Insert data from bronze tables after cleaning 	
Parameters:
    - None 
...
BEGIN
    ...
    BEGIN TRY
			...
			END AS sls_due_dt,
			case 
			when sls_sales != sls_price*sls_quantity or sls_sales <=0 
			...
			select 
				 ...
				 case 
				 when trim(upper(gen))='F' or trim(gen)='Female'  then 'Female'
				 when trim(upper(gen))='M' or trim(gen)='MAle'then 'Male'
				 else 'n/a'
				 end as 'gen'
